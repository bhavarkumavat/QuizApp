...
const main = async () => {
    ...
    const question_bank = await ((await fetch('question/module-3-question.json')).json())
    let answer_sheet = []
    ...
    view_report_button.addEventListener('click', () => {
        ...
        for (let answer of answer_sheet) {
            ...
            if (answer.selected_answer == answer.correct_answer || (answer.selected_answer + answer.tolerance) == answer.correct_answer
                || (answer.selected_answer - answer.tolerance) == answer.correct_answer) {
                total_correct++
            }
        }
        ...
        for (let answer of result.answer_sheet) {
            ...
            let accuracy_answer_label = document.createElement('p')
            ...
            report_summary_container.appendChild(accuracy_answer_label)
    ...
    next_question_button.addEventListener('click', () => {
        ...
        let selected_answer = document.querySelector("input[type='number']").value
        let correct_answer = question_bank[current_question - 1].correct_answer
        let question_id = question_bank[current_question - 1].id
        let tolerance = question_bank[current_question - 1].tolerance
